...
To run and debug Android app, at solution explorer right-click *UpcommingMovies.UI.Droid*, click "*Set as startup project*" then hit F5 and voilà! For iOS app you must do the same but *UpcommingMovies.UI.iOS* project. Don't forget for iOS, to build, run and debug from a Windows box you must been connected to a Mac and both windows and mac need to meet all requirements which could be checked at https://developer.xamarin.com/guides/cross-platform/getting_started/requirements/
